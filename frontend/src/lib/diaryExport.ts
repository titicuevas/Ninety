...
export type DiaryExportFormat = 'json' | 'csv';

/** Descarga el diario del usuario autenticado (JSON o CSV). Sin secretos. */
export async function downloadDiaryExport(
  format: DiaryExportFormat,
  accessToken: string,
): Promise<void> {
  const API_URL = resolveApiUrl();
  const path = `/api/capsules/me/export?format=${format}`;
  ...
  const filename = match?.[1] ?? `ninety-diario.${format === 'csv' ? 'csv' : 'json'}`;
