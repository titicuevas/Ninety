...
import type { CollectionLikeRow } from '@/types/like';
...
type CollectionLikersDialogProps = {
  open: boolean;
  collectionId: string;
  likesCount?: number;
  onClose: () => void;
...
export function CollectionLikersDialog({
  open,
  collectionId,
  likesCount = 0,
  onClose,
}: CollectionLikersDialogProps) {
  const { user } = useAuth();
  const {
    data,
    isLoading,
    ...
    isFetchingNextPage,
    fetchNextPage,
  } = useCollectionLikes(collectionId, open);
